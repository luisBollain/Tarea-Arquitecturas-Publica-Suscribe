...
class XiaomiMyBand:
    ...
    def publish(self):        
        ...
        print('[x] publicando valor de posicion...')
        self.draw_progress_bar(2)
        channel.basic_publish(exchange='', routing_key='position', body=str(message), properties=pika.BasicProperties(
            delivery_mode=2,))  # Se realiza la publicación del mensaje en el Distribuidor de Mensajes        
        connection.close()  # Se cierra la conexión
        print('[x] valor publicado!')
        print('')    
        ...
        message = {}
        message['medicine'] = str(self.simulate_medicine())
        message['id'] = str(self.id)
        message['datetime'] = self.simulate_datetime()
        message['producer'] = self.producer
        message['model'] = self.model
        ...
        print('[x] publicando valor de medicinas...')
        self.draw_progress_bar(2)
        channel.basic_publish(exchange='', routing_key='medicine_timer', body=str(message), properties=pika.BasicProperties(
            delivery_mode=2,))  # Se realiza la publicación del mensaje en el Distribuidor de Mensajes
        connection.close()  # Se cierra la conexión
        print('[x] valor publicado!')
        print('')  
    ...
    def simulate_medicine(self):
        medicine_id = random.randint(1,40)
        if medicine_id > 0 and medicine_id < 10:            
            return 'paracetamol'
        elif medicine_id > 10 and medicine_id < 20:            
            return 'ibuprofeno'
        elif medicine_id > 20 and medicine_id < 30:            
            return 'insulina'
        elif medicine_id > 30 and medicine_id < 40:            
            return 'tolbutamina'
